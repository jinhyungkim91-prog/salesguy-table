...
const EEL_KW   = ['장어'];
const TUNA_KW  = ['생선회','참치'];
const CRAB_KW  = ['대게','킹크랩','랍스터','꽃게','게장','해산물','해물','생선구이'];
...
function getGenreIcon(genre) {
  ...
  if (CRAB_KW.some(k => genre.includes(k))) return '🦞';
...
const GENRE_KEYWORDS = {
  '고기구이': ['고기구이','소고기구이','LA갈비','짝갈비','갈비','돼지고기구이','돼지갈비','곱창','양갈비','양고기','양대창','고깃집','소고기구이·양곱창','한우','와규'],
  ...
  '양식':    ['이탈리아','프랑스','파스타','양식','파인다이닝','웨스턴','올데이다이닝'],
};
...
const LUNCH_GENRES  = ["전체","한식·백반","국밥·해장","면류·분식","중식·마라","일식·덮밥","고기","버거","치킨","샐러드·제과","양식"];
const LUNCH_GENRE_MAP = {
  ...
  '샐러드·제과': ['샐러드','포케','건강식','비건','요거트','베이커리','빵','브레드','베이글','토스트','브런치','떡','카페·베이커리'],
...
const KAKAO_GENRE_MAP = {
  ...
  '버거':      ['햄버거','버거','패스트푸드'],
  '치킨':      ['치킨','닭강정','닭'],
  '샐러드·제과': ['샐러드','건강식','포케','베이커리','제과','브런치','떡','카페'],
  ...
  return (
    <div className="rest-card" onClick={() => onClick(r)}>
      <div className="rest-card-top">
        <div className="rest-emoji">{getGenreIcon(r.genre)}</div>
...
function DetailModal({ r, type, onClose }) {
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-box" onClick={e=>e.stopPropagation()}>
        <button className="modal-close" onClick={onClose}>✕</button>
        <div className="modal-emoji">{type === "golf" ? (r.emoji || "⛳") : getGenreIcon(r.genre)}</div>
